...
Dwie możliwości wyboru koloru przez użytkownika: RGB oraz CMYK (narzędzia do wyboru koloru mogą być wzorowane na popularnych programach graficznych),
Wybór koloru powinny odbywać się zarówno za pomocą myszy przy użyciu suwaka, jak i poprzez wprowadzenie poszczególnych wartości w pola tekstowe,
...
Wartości wybrane przez użytkownika oraz po konwersji powinny zostać wyświetlone w czasie rzeczywistym.
RGB -> CMYK:
C = (1 - R/255 - K) / (1-K)+
M = (1 - G/255 - K) / (1-K)
Y = (1 - B/255 - K) / (1-K)
...
Uwagi:
Konwersja kolorów musi być zaimplementowana ręcznie bez użycia bibliotek.
