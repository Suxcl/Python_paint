...
def interpolate_color(point):
    # Convert point from [-1,1] range to [0,1] range for RGB
    x = (point[0] + 1) / 2
    y = (point[1] + 1) / 2
    z = (point[2] + 1) / 2
    return x, y, z
...
def draw_color_section(intersection_point, normal):
    ...
    size = 2.0
    divisions = 20  # Number of divisions for color interpolation
    
    # Calculate grid of points for smooth color interpolation
    glBegin(GL_QUADS)
    for i in range(divisions):
        for j in range(divisions):
            # Calculate corners of each small quad
            x1 = i / divisions * 2 - 1
            x2 = (i + 1) / divisions * 2 - 1
            y1 = j / divisions * 2 - 1
            y2 = (j + 1) / divisions * 2 - 1
            
            points = [
                intersection_point + (x1 * right + y1 * up) * size,
                intersection_point + (x2 * right + y1 * up) * size,
                intersection_point + (x2 * right + y2 * up) * size,
                intersection_point + (x1 * right + y2 * up) * size
            ]
            
            # Draw quad with interpolated colors
            for point in points:
                # Clamp point coordinates to [-1, 1]
                clamped_point = np.clip(point, -1, 1)
                r, g, b = interpolate_color(clamped_point)
                glColor3f(r, g, b)
                glVertex3f(*point)
    glEnd()
...
def main():
    ...
    while True:
        ...
        if showing_section and intersection_point is not None and plane_normal is not None:
            draw_color_section(intersection_point, plane_normal)
        else:
            draw_cube()
